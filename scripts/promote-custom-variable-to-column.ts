...
if (!variableName) {
  console.error('❌ Error: --variable is required')
  console.log('\nUsage:')
...
async function analyzeVariable(): Promise<{ count: number; total: number; sampleValues: string[] }> {
  console.log(`\n📊 Analyzing variable: "${variableName}"`)
...
async function main() {
  console.log('🚀 Promote Custom Variable to Column')
  ...
  if (stats.count === 0) {
    console.log('\n⚠️  Warning: No records found with this variable. Are you sure the name is correct?')
  }
